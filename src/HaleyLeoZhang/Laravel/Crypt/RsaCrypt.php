<?php
namespace HaleyLeoZhang\Laravel\Crypt;
...
class RsaCrypt implements BaseCrypt
{
    /**
     * 公钥 加密
    ...
    {
        $rsa_file_public = storage_path(
            config('crypt.rsa_file_public') // 相对于 stoarge 目录的位置
        );
        $rsa_file_private = storage_path(
            config('crypt.rsa_file_private') // 相对于 stoarge 目录的位置
        );
    ...
    public static function verify($data, $sign)
    {
        ...
        $path = config('crypt.rsa_private_alipay_key_storage_path'); // 相对于 stoarge 目录的位置

        $ali_pub_key = file_get_contents(
            storage_path($path)
        );

        //转换为openssl格式密钥
        $res = openssl_get_publickey($ali_pub_key);
